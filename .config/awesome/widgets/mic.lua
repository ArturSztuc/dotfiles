...
local volume_widget = wibox.widget {
    {
        id = "icon",
        image = path_to_icons .. "microphone-sensitivity-muted-symbolic.svg",
        resize = false,
...
local update_graphic = function(widget, stdout, _, _, _)
    ...
    if mute == "off" then volume_icon_name="microphone-sensitivity-muted-symbolic_red"
    elseif (volume >= 0 and volume < 25) then volume_icon_name="microphone-sensitivity-muted-symbolic"
    elseif (volume < 50) then volume_icon_name="microphone-sensitivity-low-symbolic"
    elseif (volume < 75) then volume_icon_name="microphone-sensitivity-medium-symbolic"
    elseif (volume <= 100) then volume_icon_name="microphone-sensitivity-high-symbolic"
    end
